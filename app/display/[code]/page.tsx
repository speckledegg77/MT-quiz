                    ...
                    <div className="rounded-xl border border-[var(--border)] bg-[var(--muted)] p-3">
                      ...
                      {Array.isArray(questionStats.byTeam) && questionStats.byTeam.length ? (
                        <div className="mt-3">
                          ...
                          <div className="mt-2 grid gap-1 text-sm text-[var(--muted-foreground)]">
                            {questionStats.byTeam.map((t: any) => (
                              <div key={t.team} className="flex flex-wrap items-center gap-2">
                                <span className="font-medium text-[var(--foreground)]">{t.team}</span>
                                <span>
                                  Correct {t.correct}/{t.answered}
                                </span>
                                <span className="flex items-center gap-1">
                                  Joker {t.jokerUsed}
                                  {Number(t.jokerUsed ?? 0) > 0 ? <JokerBadge /> : null}
                                </span>
                              </div>
                            ))}
                          </div>
                  ...
                  {stage === "reveal" && roundStats && currentRound ? (
                    <div className="rounded-xl border border-[var(--border)] bg-[var(--card)] p-3">
                      <div className="text-sm font-semibold">
                        Round so far: {String(currentRound.name ?? "")}
                      </div>
                      <div className="mt-2 grid gap-1 text-sm text-[var(--muted-foreground)]">
                        <div>
