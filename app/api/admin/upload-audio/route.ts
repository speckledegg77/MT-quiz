...
export async function POST(req: Request) {
  ...
  const originalName = safeName((file as any).name || "audio")
  const folder = new Date().toISOString().slice(0, 10)
  const path = desiredPath ? desiredPath : `${folder}/${Date.now()}-${originalName}`

  const bytes = new Uint8Array(await file.arrayBuffer())

  const { error } = await supabaseAdmin.storage.from("audio").upload(path, bytes, {
  ...
  return NextResponse.json({ ok: true, bucket: "audio", path })
}
