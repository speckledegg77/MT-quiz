...
type SpotlightItem = {
  id: string
  answer_text: string
  item_type: SpotlightItemType
  person_roles: SpotlightPersonRole[] | null
...
type SpotlightItemsResponse = {
  ok: true
  items: SpotlightItem[]
}

type SpotlightPacksResponse = {
  ok: true
  packs: PackSummary[]
}
...
type ItemFormState = {
  ...
  personRoles: SpotlightPersonRole[]
  difficulty: SpotlightDifficulty
...
function toItemFormState(item: SpotlightItem): ItemFormState {
  return {
...
export function SpotlightDashboard() {
  ...
  async function loadAll(nextSelectedItemId?: string, nextSelectedPackId?: string) {
    ...
    try {
      ...
      const itemsJson = (await itemsRes.json()) as SpotlightItemsResponse | { error?: string }
      const packsJson = (await packsRes.json()) as SpotlightPacksResponse | { error?: string }
      ...
      const nextItems = (itemsJson as SpotlightItemsResponse).items ?? []
      const nextPacks = (packsJson as SpotlightPacksResponse).packs ?? []
  ...
  function updateItemType(
    nextItemType: SpotlightItemType,
    setForm: (updater: (prev: ItemFormState) => ItemFormState) => void
  ) {
    setForm((prev) => ({
      ...prev,
  ...
  async function createItemSubmit() {
    ...
    try {
      ...
      const json = (await res.json()) as { error?: string; item?: SpotlightItem }
            ...
            <Card>
              ...
              <CardContent className="grid gap-3 md:grid-cols-2 xl:grid-cols-5">
                ...
                >
                  ...
                  {SPOTLIGHT_ITEM_TYPE_VALUES.map((value) => (
                ...
                >
                  ...
                  {SPOTLIGHT_DIFFICULTY_VALUES.map((value) => (
                    <option key={value} value={value}>
                      {value}
                    </option>
                  ))}
...
type ItemFormProps = {
  ...
  onRoleToggle: (value: SpotlightPersonRole) => void
  onItemTypeChange: (value: SpotlightItemType) => void
  submitLabel: string
  onSubmit: () => void
  busy: boolean
}
    ...
    <div className="space-y-4">
      <div className="grid gap-3 md:grid-cols-2">
        ...
        <label className="space-y-1 text-sm">
          ...
          <select
            value={form.itemType}
            onChange={(event) => onItemTypeChange(event.target.value as SpotlightItemType)}
            className="h-10 w-full rounded-lg border border-border bg-background px-3 text-sm outline-none focus:ring-2 focus:ring-border"
          >
            {SPOTLIGHT_ITEM_TYPE_VALUES.map((value) => (
              <option key={value} value={value}>
        ...
        <label className="space-y-1 text-sm">
          ...
          <select
            ...
            onChange={(event) =>
              setForm((prev) => ({
                ...prev,
                difficulty: event.target.value as SpotlightDifficulty,
              }))
            }
            className="h-10 w-full rounded-lg border border-border bg-background px-3 text-sm outline-none focus:ring-2 focus:ring-border"
          >
      ...
      {form.itemType === "person" ? (
        <div className="space-y-2">
          ...
          <div className="grid gap-2 sm:grid-cols-2">
            {SPOTLIGHT_PERSON_ROLE_VALUES.map((value) => {
