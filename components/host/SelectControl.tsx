...
export default function SelectControl({ children, className = "", compact = false, variant = "default", ...props }: SelectControlProps) {
  ...
  const isAdvanced = resolvedVariant === "advanced"

  const sizeClasses = isAdvanced
    ? compact
  ...
  const variantClasses = isAdvanced
  ...
  const iconSizeClasses = isAdvanced
    ? "h-3.5 w-3.5"
    : isToolbar
      ? compact
        ? "h-3.5 w-3.5"
  ...
  const inlineStyle: CSSProperties | undefined = isAdvanced
    ? {
