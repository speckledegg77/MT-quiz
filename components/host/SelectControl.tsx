...
export default function SelectControl({ children, className = "", compact = false, variant = "default", ...props }: SelectControlProps) {
  ...
  const isQuiet = resolvedVariant === "soft" || resolvedVariant === "advanced"
  ...
  const variantClasses = isToolbar
    ...
    : isQuiet
      ? "border border-border/40 bg-background text-foreground shadow-none hover:border-border/70 focus:border-foreground/20 focus:ring-1 focus:ring-foreground/10"
  ...
  const iconSizeClasses = isToolbar || isQuiet
    ? compact
      ? "h-3.5 w-3.5"
      : "h-3.5 w-3.5"
    : "h-4 w-4"
