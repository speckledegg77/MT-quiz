...
export default function HostJoinedTeamsPanel({ code }: { code: string }) {
  ...
  if (loading) {
    return (
      <Card>
        <CardContent className="py-4 text-sm text-muted-foreground">Loading players…</CardContent>
      </Card>
    );
  }
  ...
  const nameForRound = (idx: number) => {
    if (idx >= 0 && idx < roundNames.length) return roundNames[idx];
    return `Round ${idx + 1}`;
  };
    ...
    <div className="space-y-4">
      <Card>
        <CardContent className="p-0">
          {error ? (
            <div className="border-b border-border px-4 py-3 text-sm text-red-600 dark:text-red-300">
              {error}
            </div>
          ) : null}
                  ...
                  {sorted.map((p) => {
                    ...
                    return (
                      <tr key={p.id} className="border-b border-border last:border-b-0">
                        ...
                        <td className="px-3 py-2 text-muted-foreground">{String(p.team_name ?? "")}</td>
