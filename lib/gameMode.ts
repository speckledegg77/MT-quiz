...
export function getStageStatusText(stage: GameStage, isInfiniteFinalStage = false) {
  ...
  if (stage === "spotlight_ready") return "Ready for turn"
  if (stage === "spotlight_live") return "Turn live"
  if (stage === "spotlight_review") return "Turn review"
...
export function getStagePillClass(stage: GameStage) {
  if (stage === "open" || stage === "spotlight_live") return "bg-emerald-600/20 text-emerald-200 border-emerald-500/40"
  if (stage === "reveal" || stage === "spotlight_review") return "bg-indigo-600/20 text-indigo-200 border-indigo-500/40"
  if (stage === "round_summary") return "bg-violet-600/20 text-violet-200 border-violet-500/40"
  if (stage === "spotlight_ready") return "bg-amber-600/20 text-amber-200 border-amber-500/40"
