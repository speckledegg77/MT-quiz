...
- Stage 4C changes new room round plans and synthetic Spotlight card ids to write `spotlight` values.
- Stage 4D renames the database tables to `spotlight_items`, `spotlight_packs`, and `spotlight_pack_items`.
- Stage 4E-B removes the final Spotlight persistence bridge by renaming `rooms.heads_up_state` to `rooms.spotlight_state`, dropping the temporary `heads_up_*` compatibility views, and removing legacy Spotlight read fallbacks from the app layer.
...
- Spotlight gameplay starts as a manual-round option only, using one Spotlight pack per round.
- Spotlight uses role-based live views. The guesser sees only the timer plus Correct and Pass. Clue-givers see the live clue plus timer.
...
- Spotlight rounds support a round-level TV display toggle: show clue on TV or timer only. The default is timer only.
- Spotlight rounds support 60 second and 90 second turn lengths, with 60 seconds as the default.
- Spotlight scoring is recorded by the active guesser on their phone, with host undo and end-of-turn review before the turn is confirmed.
...
- During the full Spotlight rename, app routes and API paths should move to `spotlight` names first, while legacy `heads-up` paths stay alive only as temporary compatibility aliases until the app-layer cleanup removes them. The full Spotlight migration should leave no live `heads_up` compatibility layer in the active app or schema.
