...
- During the full Spotlight rename, app routes and API paths should move to `spotlight` names first, while legacy `heads-up` paths stay alive only as temporary compatibility aliases until the app-layer cleanup removes them. The same applies to legacy `heads_up_*` table names, which should exist only as temporary compatibility views after the schema rename.
